...
export default function ReservationsPage() {
  ...
  async function handleSave() {
    ...
    setSaving(true)
    const { id, created_at, created_by, updated_by: _ub, updated_at: _ua, biens: _b, ...fields } = editing as any
    const isNew = !editing.id
    if (editing.id) {
      const original = rows.find((r) => r.id === editing.id)
      await supabase.from('reservations').update({ ...fields, updated_by: userEmail, updated_at: new Date().toISOString() }).eq('id', editing.id)
                ...
                <tr key={r.id} className={`transition-colors ${isCheckinDemain(r) ? 'bg-blue-50/60 border-l-2 border-l-blue-400' : isCheckoutDemain(r) ? 'bg-orange-50/60 border-l-2 border-l-orange-400' : isEnCours(r) ? 'bg-green-50/60 border-l-2 border-l-green-400' : 'hover:bg-creme/40'} ${selected.has(r.id) ? 'bg-terra/5' : ''}`}>
                  ...
                  <td className="px-3 py-3">
                    <div className="flex gap-1">
                      <button onClick={() => openEdit(r)} title="Modifier" className="w-7 h-7 flex items-center justify-center rounded-lg text-terra hover:bg-terra/10 transition-all">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
                      </button>
                      <button onClick={() => generateFacture(r)} title="Facture" className="w-7 h-7 flex items-center justify-center rounded-lg text-brun-mid hover:bg-brun/10 transition-all">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="5" y="2" width="14" height="20" rx="2" /><path d="M9 7h6M9 11h6M9 15h4" strokeLinecap="round" /></svg>
                      </button>
                      <button onClick={() => { setHistoriqueOpen(true); fetchHistorique(r.id) }} title="Historique" className="w-7 h-7 flex items-center justify-center rounded-lg text-brun-mid/60 hover:bg-brun/10 transition-all">
                      ...
                      <button onClick={() => openDuplicate(r)} title="Dupliquer" className="w-7 h-7 flex items-center justify-center rounded-lg text-brun-mid hover:bg-brun/10 transition-all">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="9" y="9" width="13" height="13" rx="2" /><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1" /></svg>
                      </button>
                      <button onClick={() => handleDelete(r.id)} title="Supprimer" className="w-7 h-7 flex items-center justify-center rounded-lg text-red-400 hover:bg-red-50 transition-all">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6" /><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a1 1 0 011-1h4a1 1 0 011 1v2" /></svg>
                      </button>
                    </div>
                  </td>
