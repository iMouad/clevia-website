          ...
          <table className="w-full text-sm">
            <thead className="bg-brun/4">
              <tr>
                {['Date & heure', 'Bien', 'Téléphone', 'Appareil', 'Localisation', 'Lien'].map((h) => (
                  <th key={h} className="px-4 py-3 text-left text-xs text-brun-mid uppercase tracking-wide font-medium whitespace-nowrap">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-brun/5">
              {(data ?? []).map((c: any) => {
                ...
                const localisation = c.ville_geo && c.pays
                  ? `${c.ville_geo}, ${c.pays}`
                  : c.pays ?? '—'
                ...
                return (
                  <tr key={c.id} className="hover:bg-creme/40 transition-colors">
                    ...
                    <td className="px-4 py-3 text-brun font-medium max-w-[200px] truncate">
                      {c.bien_titre ?? '—'}
                    </td>
                    <td className="px-4 py-3 text-brun-mid whitespace-nowrap">
                      {c.telephone ?? '—'}
                    </td>
                    <td className="px-4 py-3 text-brun-mid capitalize whitespace-nowrap">
                      {c.appareil ?? '—'}
                    </td>
                    <td className="px-4 py-3 text-brun-mid whitespace-nowrap">
                      {localisation}
              ...
              {!data?.length && (
                <tr>
                  <td colSpan={6} className="px-4 py-12 text-center text-brun-mid/50 text-sm">
